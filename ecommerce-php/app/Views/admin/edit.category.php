...
<head>
  ...
  <title>Editar Categoria - Um Convite de Casamento</title>
    ...
    <div class="flex-1 p-4 md:p-6 space-y-6 min-w-0">
      ...
      <div class="w-full max-w-xl mx-auto bg-white p-4 sm:p-6 md:p-8 rounded-xl shadow space-y-5">
        <div class="flex items-center gap-3">
          ...
          <h3 class="text-base font-semibold text-gray-800">Editar Categoria</h3>
        </div>
        ...
        <?php elseif (!empty($sucesso)): ?>
          <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded" role="alert">
            <p class="text-sm"><?php echo htmlspecialchars($sucesso, ENT_QUOTES, 'UTF-8'); ?></p>
          </div>
        <?php endif; ?>

        <?php
        $mode = 'edit';
        $action = url('admin/categorias/editar') . '?id=' . (int) $categoria_id;
